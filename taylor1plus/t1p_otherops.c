...
t1p_t* t1p_forget_array(ap_manager_t* man, bool destructive, t1p_t* a,
...
t1p_t t1p_expand(ap_manager_t* man, bool destructive, t1p_t* a, ap_var_t var,
...
t1p_t t1p_fold(ap_manager_t* man, bool destructive, t1p_t* a, ap_var_t* tvar,
		size_t size) {
	CALL();
...
t1p_t* t1p_widening(ap_manager_t* man, t1p_t* a1, t1p_t* a2) {
	...
	if (t1p_is_eq(man, a1, a2)) {
		res = t1p_copy(man, a1);
	} else if (tbool_or(t1p_is_top(man, a1), t1p_is_top(man, a2))
			== tbool_true) {
		...
		for (i = 0; i < (intdim + realdim); i++)
			itv_widening(res->box[i], a1->box[i], a2->box[i]);
		for (i = 0; i < (intdim + realdim); i++) {
			if (t1p_aff_is_bottom(pr, a1->paf[i]))
				res->paf[i] = a2->paf[i];
			else if (t1p_aff_is_bottom(pr, a2->paf[i]))
				res->paf[i] = a1->paf[i];
			else if (t1p_aff_is_top(pr, a1->paf[i])
					|| t1p_aff_is_top(pr, a2->paf[i]))
				res->paf[i] = pr->top;
				...
				if (itv_has_infty_bound(res->box[i])) {
					...
					res->paf[i] = t1p_aff_widening_constrained6(pr, a1->paf[i],
							a2->paf[i], a1, a2, res);
				}
			}
			res->paf[i]->pby++;
		...
		if (a1->hypercube && a2->hypercube) {
			for (i = 0; i < (intdim + realdim); i++) {
				if (t1p_aff_is_bottom(pr, a1->paf[i]))
					...
					if (itv_has_infty_bound(a1->box[i])
							|| itv_has_infty_bound(a2->box[i])) {
						/* Do nothing, the join of concretisations is already done and stored in res->box */
						res->paf[i] = t1p_aff_alloc_init(pr);
						itv_set(res->paf[i]->c, res->box[i]);
					} else {
						...
						itv_set(a1->paf[i]->itv, a1->box[i]);
						itv_set(a2->paf[i]->itv, a2->box[i]);
						res->paf[i] = t1p_aff_widening_constrained6(pr,
								a1->paf[i], a2->paf[i], a1, a2, res);
					}
				}
				res->paf[i]->pby++;
			}
			...
			if (dims1 && dims2) {
				...
				if (dims1 > res->size) {
					res->nsymcons = (ap_dim_t*) realloc(res->nsymcons,
							(dims1) * sizeof(ap_dim_t));
					res->gamma = (ap_interval_t**) realloc(res->gamma,
							(dims1) * sizeof(ap_interval_t*));
					for (k = res->size; k < dims1; k++)
						res->gamma[k] = NULL;
					res->size = dims1;
				}
				res->nsymcons = memcpy((void *) res->nsymcons,
						(const void *) a1->nsymcons, dims1 * sizeof(ap_dim_t));
				for (i = 0; i < dims1; i++)
				...
				ap_abstract0_add_dimensions(pr->manNS, true, a2->abs,
						dimchange2, false);
				...
				ap_abstract0_remove_dimensions(pr->manNS, true, a2->abs,
						dimchange2);
				...
				pr->dimtoremove = (ap_dim_t*) realloc(pr->dimtoremove,
...
void ap_abstract1_aff_build(ap_manager_t* man, ap_abstract1_t* abstract1,
		ap_var_t var, unsigned int index, ap_interval_t *interval, bool isunion)
// TODO TODO TODO : est ce que cette fonction sert tjs ?
{
	...
	if (pr->dim <= index)
		t1p_aff_nsym_create(pr, abs->paf[dim], itv, isunion ? UN : IN);
	else
		t1p_aff_build(pr, abs->paf[dim], itv, index);
...
void ap_abstract1_ns_meet_box_array(ap_manager_t* man,
		ap_abstract1_t* abstract1, ap_interval_t** box, size_t size) {
	CALL();
	size_t i = 0;
	t1p_internal_t* pr = (t1p_internal_t*) man->internal;
	...
	if (size != pr->dim)
		fatal("dimension of box and number of used noise symbols mismatch\n");
