...
t1p_t* t1p_of_box(ap_manager_t* man, size_t intdim, size_t realdim,
		ap_interval_t** tinterval) {
	CALL();
	t1p_internal_t* pr = t1p_init_from_manager(man, AP_FUNID_OF_BOX);
	itv_t* itv_array = itv_array_alloc(intdim + realdim);
	itv_array_set_ap_interval_array(pr->itv, &itv_array, tinterval,
			intdim + realdim);
	t1p_t* res = t1p_alloc(man, intdim, realdim);
	size_t i = 0;
	for (i = 0; i < intdim + realdim; i++) {
		...
		if (itv_is_bottom(pr->itv, itv_array[i]))
			...
			t1p_aff_add_itv(pr, res->paf[i], itv_array[i], IN);
		else
			itv_set(res->paf[i]->c, itv_array[i]);
		res->paf[i]->pby++;
	}
...
//    t1p_fprint(stdout, man, a, 0x0);
	...
	if (t1p_is_bottom(man, a) == tbool_true)
		res = tbool_true;
	else if (t1p_is_bottom(man, b) == tbool_true)
		res = tbool_false;
	else if (t1p_is_top(man, b) == tbool_true)
		res = tbool_true;
	else if (t1p_is_top(man, a) == tbool_true)
		res = tbool_false;
	else if (t1p_is_eq(man, a, b))
		res = tbool_true;
	else {
		/* General case */
		ok = true;
				...
				for (i = 0; i < sizea; i++) {
					if (t1p_nsymcons_get_dimpos(pr, &dima, i, a)) {
						if (t1p_nsymcons_get_dimpos(pr, &dimb, i, b)) {
							if (ap_interval_is_leq(a->gamma[dima],
									b->gamma[dimb])) {
								ok &= true;
							} else {
								ok &= false;
		...
		if (ok) {
			for (i = 0; i < a->dims; i++) {
				if (!itv_is_leq(a->box[i], b->box[i])) {
					...
					ok &= t1p_aff_is_leq_constrained(pr, a->paf[i], b->paf[i],
							a, b);
				}
...
tbool_t t1p_is_dimension_unconstrained(ap_manager_t* man, t1p_t* a,
		ap_dim_t dim) {
	CALL();
	t1p_internal_t* pr = t1p_init_from_manager(man,
...
ap_interval_t* t1p_bound_linexpr(ap_manager_t* man, t1p_t* a,
		ap_linexpr0_t* expr) {
...
ap_interval_t** t1p_to_box(ap_manager_t* man, t1p_t* a) {
	...
	ap_interval_t **res = (ap_interval_t**) malloc(
			a->dims * sizeof(ap_interval_t*));
	size_t i = 0;
	//itv_t tmp; itv_init(tmp);
	for (i = 0; i < a->dims; i++) {
