...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_polyhedron(&polyhedron1, man, top, &number1, data, dataSize,
			...
			pk_t* joinbottom1 = pk_join(man, DESTRUCTIVE, bottom,
					polyhedron1);
			pk_internal_t * joinbottom1_internal = pk_init_from_manager(
					man, AP_FUNID_JOIN);

			if (joinbottom1_internal->exn != AP_EXC_OVERFLOW) {

				//meet == glb, join == lub
				//bottom join x == x
				if (pk_is_eq(man, joinbottom1, polyhedron1) == false) {
