 ...
 */
bool poly_meet_matrix(bool meet, bool lazy, ap_manager_t* man, pk_t* po,
	...
	/* strict behaviour */
		...
		if (po != pa) {
			...
			satmat_resize_cols(pa->satC, bitindex_size(pa->C->nbrows));
		}
		cherni_add_and_minimize(pk, meet, po, start);

		if (pk->exn)
			goto _poly_meet_matrix_exit0;
		po->status = meet ? (pk_status_consgauss) : (pk_status_gengauss);
		assert(poly_check_dual(pk, po, meet));
	}
	return false;
	_poly_meet_matrix_exit0: po->status = 0;
	{
		char str[160];
...
/* Factorized form */
/* ---------------------------------------------------------------------- */
static bool poly_meet_particularcases(bool meet, bool lazy, ap_manager_t* man,
...
void poly_meet(bool meet, bool lazy, ap_manager_t* man, pk_t* po, pk_t* pa,
		pk_t* pb) {
	pk_internal_t* pk = (pk_internal_t*) man->internal;
	int pa_status;
	...
	/* lazy behaviour */
	if (lazy) {
		_poly_meet_entry0: poly_obtain_sorted_C(pk, pa);
	...
	/* strict behaviour */
		...
		if (po != pa) {
			...
			if (pa->F && !pb->F) /* only pa is minimized */
				...
				if (!pa->F && !pb->F) {
					/* ensure minimization */
					poly_chernikova_dual(man, pa, "of the first argument",
							meet);
					if (pk->exn) {
						assert(pa->C);
						pk->exn = AP_EXC_NONE;
					...
					poly_chernikova_dual(man, pb, "of the second argument",
							meet);
					if (pk->exn) {
						assert(pb->C);
						pk->exn = AP_EXC_NONE;
...
static pk_t* poly_meet_array(bool meet, bool lazy, ap_manager_t* man, pk_t** po,
		size_t size) {
	...
	/* 1. Special cases */
	if (size == 0) {
		ap_manager_raise_exception(man, AP_EXC_INVALID_ARGUMENT, pk->funid,
				"empty array");
		man->result.flag_best = man->result.flag_exact = false;
		poly = pk_top(man, 0, 1);
		if (!meet)
	...
	/* 2. General case */
		...
		/* 2.2 strict hehaviour */
			...
			if (pk->exn)
				goto _poly_meet_array_exit0;
			poly->status = meet ? (pk_status_consgauss) : (pk_status_gengauss);
		}
		assert(poly_check_dual(pk, poly, meet));
		return poly;
		_poly_meet_array_exit0: poly->status = 0;
		{
			char str[160];
			sprintf(str, "conversion from %s of the (intermediate) result\n",
					(meet ? poly->C : poly->F) ?
...
pk_t* pk_meet(ap_manager_t* man, bool destructive, pk_t* pa, pk_t* pb) {
	pk_internal_t* pk = pk_init_from_manager(man, AP_FUNID_MEET);
	pk_t* po = destructive ? pa : poly_alloc(pa->intdim, pa->realdim);
	poly_meet(true, pk->funopt->algorithm < 0, man, po, pa, pb);
	return po;
}

pk_t* pk_meet_array(ap_manager_t* man, pk_t** po, size_t size) {
	pk_internal_t* pk = pk_init_from_manager(man, AP_FUNID_MEET_ARRAY);
	pk_t* res = poly_meet_array(true, pk->funopt->algorithm < 0, man, po, size);
...
/* ---------------------------------------------------------------------- */
/* Factorized version */
void poly_meet_itv_lincons_array(bool lazy, ap_manager_t* man, pk_t* po,
		pk_t* pa, itv_lincons_array_t* array) {
	matrix_t* mat;
	bool quasilinear;
	pk_internal_t* pk = (pk_internal_t*) man->internal;
	...
	bool exact = matrix_set_itv_lincons_array(pk, &mat, array, po->intdim,
			po->realdim, true);
...
void* pk_meet_lincons_array(ap_manager_t* man, bool destructive, void* ppa,
	...
	poly_meet_itv_lincons_array(pk->funopt->algorithm < 0, man, po, pa, &tcons);
	itv_lincons_array_clear(&tcons);
	assert(poly_check(pk, po));
	return (void*) po;
}

pk_t* pk_meet_tcons_array(ap_manager_t* man, bool destructive, pk_t* pa,
...
pk_t* pk_join(ap_manager_t* man, bool destructive, pk_t* pa, pk_t* pb) {
	...
	poly_meet(false, pk->funopt->algorithm < 0, man, po, pa, pb);
	poly_dual(pa);
	if (pb != pa)
		poly_dual(pb); /* We take care of possible alias */
	if (po != pa)
	...
	if (po->F && (po->F->nbrows < po->F->_maxrows)) {
		...
		for (i = 0; i < nbrows; i++) {
			if (vector_is_positivity_constraint(pk, po->F->p[i],
					po->F->nbcolumns)) {
				flag = false;
			}
		}
	...
	pk_t* pk_join_array(ap_manager_t* man, pk_t** po, size_t size) {
		...
		for (i = 0; i < size - 1; i++) {
			if (tpoly[i] == tpoly[i + 1]) {
				if (i < size - 2) {
					memmove(&tpoly[i + 1], &tpoly[i + 2],
							(size - i - 2) * sizeof(pk_t*));
				}
				size--;
			}
		}
		...
		poly = poly_meet_array(false, pk->funopt->algorithm < 0, man,
				(pk_t**) tpoly, size);
		for (i = 0; i < size; i++) {
			poly_dual(tpoly[i]);
		}
	...
	/* Factorized version */
	static
	void poly_add_ray_array(bool lazy, ap_manager_t* man, pk_t* po, pk_t* pa,
		...
		exact = matrix_set_ap_generator0_array(pk, &mat, array, pa->intdim,
				pa->realdim);
		matrix_sort_rows(pk, mat);
	...
	pk_t* pk_add_ray_array(ap_manager_t* man, bool destructive, pk_t* pa,
			ap_generator0_array_t* array) {
