...
bool create_octogonal_constraint(ap_linexpr0_t** constraint, ap_constyp_t *type,
...
bool create_constraints(ap_lincons0_array_t *lincons0, long nbcons, int dim,
	...
	*lincons0 = ap_lincons0_array_make(nbcons);
	...
	size_t i;
	for (i = 0; i < nbcons; i++) {
		if (!create_octogonal_constraint(&constraint, &type, dim, randomVariable, data,
...
bool create_conditional(ap_lincons0_array_t *conditionalExpression, int dim,
		const long *data, size_t dataSize, unsigned int *dataIndex, FILE *fp) {
	ap_constyp_t type;
	...
	*conditionalExpression = ap_lincons0_array_make(1);
