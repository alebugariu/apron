...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out3.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			if (get_element(&element2, man, top, &number2, data, dataSize,
				...
				if (get_element(&element3, man, top, &number3, data,
						dataSize, &dataIndex, fp)) {

					// <= is transitive
					if (assume_fuzzable(
							ap_abstract0_is_leq(man, element1, element2)
									&& ap_abstract0_is_leq(man, element2,
											element3))) {
						if (ap_abstract0_is_leq(man, element1, element3)
								== false) {
