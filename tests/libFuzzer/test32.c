...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out32.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			ap_abstract0_t* wideningResult;
			int i = 0;
			while (true) {
				ap_abstract0_t* element2;
				unsigned char number2;
				if (get_element(&element2, man, top, &number2, data,
						dataSize, &dataIndex, fp)) {

					wideningResult = ap_abstract0_widening(man, element1,
							ap_abstract0_join(man, DESTRUCTIVE, element1,
									element2));
					if (ap_abstract0_is_leq(man, wideningResult, element1)
							== true) {
						break; // we reached a fixed point
					}
					element1 = wideningResult;
					i++;
					if (!(R(i))) {
