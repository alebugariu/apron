...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out30.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			if (assume_fuzzable(ap_abstract0_is_leq(man, bottom, element1))) {
				//meet == glb, join == lub
				//bottom widening x == x
				if (ap_abstract0_is_eq(man, ap_abstract0_widening(man, bottom, element1),
						element1) == false) {
