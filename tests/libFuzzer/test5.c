...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out5.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			ap_abstract0_t* joinbottom1 = ap_abstract0_join(man, DESTRUCTIVE,
					bottom, element1);

			//meet == glb, join == lub
			//bottom join x == x
			if (ap_abstract0_is_eq(man, joinbottom1, element1) == false) {
				...
				ap_abstract0_free(man, joinbottom1);
				ap_manager_free(man);
				fclose(fp);
				return 1;
			}
