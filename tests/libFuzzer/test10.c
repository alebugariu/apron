...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out10.txt"), "w+");
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			if (get_element(&element2, man, top, &number2, data, dataSize,
				...
				if (get_element(&element3, man, top, &number3, data, dataSize,
					...
					ap_abstract0_t* join12 = ap_abstract0_join(man, DESTRUCTIVE,
							element1, element2);

					ap_abstract0_t* join23 = ap_abstract0_join(man, DESTRUCTIVE,
							element2, element3);

					ap_abstract0_t* join12_3 = ap_abstract0_join(man,
							DESTRUCTIVE, join12, element3);

					ap_abstract0_t* join1_23 = ap_abstract0_join(man,
							DESTRUCTIVE, element1, join23);

					//meet == glb, join == lub
					//join is associative
					if (ap_abstract0_is_eq(man, join12_3, join1_23) == false) {
						...
						ap_abstract0_free(man, join12);
						ap_abstract0_free(man, join12_3);
						ap_abstract0_free(man, join23);
						ap_abstract0_free(man, join1_23);
						ap_manager_free(man);
						fclose(fp);
						return 1;
					}
