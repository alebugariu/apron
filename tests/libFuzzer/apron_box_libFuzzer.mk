...
test:
    ...
    number=$(start) ; while [ $${number} -le $(number) ] ; do \
    	rm MY_CORPUS/* ; \
	    clang -lstdc++ -fsanitize-coverage=trace-pc-guard -I /usr/local/include -I $(APRON) -I $(BOX) $(LIB_PATHS) test.c test$${number}.c \
        /home/libFuzzer.a -o test$${number} -lmpfr -lgmp -lapron -litv $(lib) -DAP_BOX; \
		startTime=`date +%s%N` ; \
        ...
        echo "Execution time: $$runtime millisec for test_box$$number\n" ; \
