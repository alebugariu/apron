...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out29.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			if (assume_fuzzable(ap_abstract0_is_leq(man, element1, bottom))) {
				//meet == glb, join == lub
				//x widening bottom == x
				if (ap_abstract0_is_eq(man, ap_abstract0_widening(man, element1, bottom),
						element1) == false) {
