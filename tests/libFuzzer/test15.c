...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out15.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			ap_abstract0_t* meetbottom1 = ap_abstract0_meet(man, DESTRUCTIVE,
					bottom, element1);

			//meet == glb, join == lub
			//bottom meet x == bottom
			if (!ap_abstract0_is_bottom(man, meetbottom1)) {
				...
				ap_abstract0_free(man, meetbottom1);
				ap_manager_free(man);
				fclose(fp);
				return 1;
			}
