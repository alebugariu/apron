...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out19.txt"), "w+");
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			if (get_element(&element2, man, top, &number2, data, dataSize,
				...
				ap_abstract0_t* meet12 = ap_abstract0_meet(man, DESTRUCTIVE,
						element1, element2);
				ap_abstract0_t* meet21 = ap_abstract0_meet(man, DESTRUCTIVE,
						element2, element1);

				//meet == glb, join == lub
				//meet is commutative
				if (ap_abstract0_is_eq(man, meet12, meet21) == false) {
