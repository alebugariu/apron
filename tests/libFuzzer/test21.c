...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out21.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			ap_abstract0_t* meet11 = ap_abstract0_meet(man, DESTRUCTIVE,
					element1, element1);

			//meet == glb, join == lub
			//meet is idempotent
			if (ap_abstract0_is_eq(man, meet11, element1) == false) {
