...
extern int LLVMFuzzerTestOneInput(const long *data, size_t dataSize) {
	...
	FILE *fp;
	fp = fopen(create_fileName("out26.txt"), "w+");
	...
	if (create_pool(man, top, bottom, dim, data, dataSize, &dataIndex, fp)) {
		...
		if (get_element(&element1, man, top, &number1, data, dataSize,
			...
			if (get_element(&element2, man, top, &number2, data, dataSize,
				...
				ap_abstract0_t* glb = ap_abstract0_meet(man, DESTRUCTIVE,
						element1, element2);
				...
				if (get_element(&bound, man, top, &number3, data, dataSize,
					...
					if (assume_fuzzable(
							ap_abstract0_is_leq(man, bound, element1))) {
						if (assume_fuzzable(
								ap_abstract0_is_leq(man, bound, element2))) {
							if (ap_abstract0_is_leq(man, bound, glb) == false) {
