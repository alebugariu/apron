...
if (man->result.exn!=AP_EXC_NONE){ 
value v = camlidl_c2ml_manager_struct_ap_exclog_t(man->result.exclog,_ctx);
	...
	value _v_v)
{
  ...
  camlidl_free(_ctx);
  /* begin user-supplied deallocation sequence */
ap_interval_free(_res); if (man->result.exn!=AP_EXC_NONE){ 
value v = camlidl_c2ml_manager_struct_ap_exclog_t(man->result.exclog,_ctx);
caml_raise_with_arg(*caml_named_value("apron exception"),v);
}
...
_res.size = dim.intdim + dim.realdim;
  ...
  camlidl_free(_ctx);
  /* begin user-supplied deallocation sequence */
ap_interval_array_free(_res.p, _res.size); if (man->result.exn!=AP_EXC_NONE){ 
value v = camlidl_c2ml_manager_struct_ap_exclog_t(man->result.exclog,_ctx);
caml_raise_with_arg(*caml_named_value("apron exception"),v);
}
	...
	value _v_a)
{
  ...
  camlidl_free(_ctx);
  /* begin user-supplied deallocation sequence */
free(_res.p); if (man->result.exn!=AP_EXC_NONE){ 
value v = camlidl_c2ml_manager_struct_ap_exclog_t(man->result.exclog,_ctx);
caml_raise_with_arg(*caml_named_value("apron exception"),v);
}
	...
	value _v_v1)
{
  ...
  v1 = Int_val(_v_v1);
  /* begin user-supplied calling sequence */
_res = ap_abstract0_add_dimensions(man,false,a,&dimchange, v1);ap_dimchange_clear(&dimchange);
  /* end user-supplied calling sequence */
  _vres = camlidl_c2ml_abstract0_ap_abstract0_ptr(&_res, _ctx);
  camlidl_free(_ctx);
  /* begin user-supplied deallocation sequence */
